...
void initEmployeeManager(EmployeeManager* pEmployeeManager)
{
    ...
    pEmployeeManager->employeeCount = False;
}
...
void sortEmployees(EmployeeManager* emmployeeManager)
{
    ...
    switch (emmployeeManager->sortField)
    {
        case eEmployeesRole:
...
void findEmployee(const EmployeeManager* pManager) {
    if (pManager->sortField == eNone) {
    ...
    switch (pManager->sortField) {
        case eEmployeesRole:
            printf("Search by Role: ");
            setEmployeeRole( &tmpEmployee);
            found = bsearch(&pTmpEmployee, pManager->EmployeeArr, pManager->employeeCount, sizeof(Employee*), compareByRole);
            break;
        case eName:
            printf("Search by Name: ");
            setEmployeeName(&tmpEmployee.name);
            found = bsearch(&pTmpEmployee, pManager->EmployeeArr, pManager->employeeCount, sizeof(Employee*), compareByName);
            break;
        case eId:
            printf("Search by ID: ");
            returnEmployeeId(&tmpEmployee);
            found = bsearch(&pTmpEmployee, pManager->EmployeeArr, pManager->employeeCount, sizeof(Employee*),compareById);
            break;
    }
